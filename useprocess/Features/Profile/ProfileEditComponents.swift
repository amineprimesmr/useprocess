...
// MARK: - Commission promo (Opal-style, hub Réglages)

struct ProcessSettingsCommissionPromoSection: View {
    @Bindable private var referralStore = ProcessReferralStore.shared

    static func shouldShow(stats: ProcessReferralCommissionStats) -> Bool {
        stats.lifetimeCents == 0 && stats.activeSubscribers == 0
    }
    ...
    var body: some View {
        if showsPromo {
            ...
            .padding(.bottom, ProcessSettingsOpalTheme.cardSpacing)
            .onAppear {
    ...
    private var showsPromo: Bool {
        Self.shouldShow(stats: referralStore.snapshot.commissionStats)
    }
    ...
    private var ctaTitle: String {
        AppCopy.t(
            "Commencer — \(ProcessReferralProgramTerms.commissionPerPaymentLabel)/ami",
            en: "Start — \(ProcessReferralProgramTerms.commissionPerPaymentLabel)/friend"
        )
    }
