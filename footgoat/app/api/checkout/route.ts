...
export async function POST(request: Request) {
  ...
  const owner = parseNickname(body.nickname ?? "");
  if (!owner) {
    return NextResponse.json({ error: "NEED_NICK" }, { status: 400 });
  }

  try {
    const locale = body.locale === "en" ? "en" : "fr";
    const meta = await resolveMeta(target, locale);
    const listingKey = meta.canonicalKey || target.key;
    const listingUrl = meta.canonicalUrl || target.url;
    ...
    const existing = await findListingByKey(listingKey);
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      custom_text: {
        ...
        after_submit: {
          message:
            locale === "en"
              ? "You can raise later with the same player — pay the full new bid amount."
              : "Tu pourras monter plus tard avec le même joueur — paie le montant d'enchère en entier.",
        },
      },
      metadata: {
      ...
      payment_intent_data: {
        ...
        description: `footgoat · ${title} · $${targetEuros}`,
      },
      line_items: [
        {
          quantity: 1,
          price_data: {
            ...
            product_data: {
              name: locale === "en" ? `footgoat rank · ${title}` : `Rang footgoat · ${title}`,
              description: bidLine,
              images: icon.startsWith("https://")
                ? [icon]
                : icon.startsWith("/players/")
                  ? [`${origin}${icon}`]
                  : undefined,
            },
          },
