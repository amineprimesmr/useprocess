...
exports.affiliateTikTokStudio = exports.affiliateTikTokOAuthCallback = void 0;
exports.tiktokApiReady = tiktokApiReady;
exports.listPublicTikTokAccounts = listPublicTikTokAccounts;
exports.tiktokTotals = tiktokTotals;
exports.hashMcpToken = hashMcpToken;
exports.loadStudio = loadStudio;
...
function tiktokApiReady() {
    return tiktokEnv().ready;
}
function oauthScopes() {
    if (process.env.TIKTOK_OAUTH_SCOPES)
        return process.env.TIKTOK_OAUTH_SCOPES;
...
function accountsCol(affiliateId) {
    return (0, affiliateShared_1.db)().collection("affiliates").doc(affiliateId).collection("tiktokAccounts");
}
function postsCol(affiliateId) {
    return (0, affiliateShared_1.db)().collection("affiliates").doc(affiliateId).collection("studioPosts");
}
function keysCol(affiliateId) {
    return (0, affiliateShared_1.db)().collection("affiliates").doc(affiliateId).collection("mcpKeys");
}
function publicAccount(id, data) {
    const row = data || {};
    return {
        id,
...
function hashMcpToken(raw) {
    return crypto.createHash("sha256").update(raw).digest("hex");
}
function publicPost(id, data) {
    return {
        id,
...
async function loadStudio(affiliateId) {
...
async function refreshAccountStats(affiliateId, accountId) {
    ...
    if (!token)
        return publicAccount(accountId, snap.data());
...
function redirectStudio(query) {
    return `${SITE}#/automatisation?${query}`;
}
exports.affiliateTikTokOAuthCallback = (0, https_1.onRequest)({
    invoker: "public",
    cors: true,
    timeoutSeconds: 30,
    memory: "256MiB",
    ...
    try {
        const stateSnap = await (0, affiliateShared_1.db)().collection("affiliateOAuthStates").doc(state).get();
        if (!stateSnap.exists) {
        ...
        else if (raw.includes("invalid_grant") || raw.includes("invalid_code"))
            codeName = "invalid_grant";
        res.redirect(redirectStudio(`tab=connexions&error=${encodeURIComponent(codeName)}`));
    }
});
exports.affiliateTikTokStudio = (0, https_1.onRequest)({
    invoker: "public",
    cors: true,
    timeoutSeconds: 30,
    memory: "512MiB",
    ...
    try {
        ...
        if (action === "publishNow") {
            ...
            if (!tiktokApiReady() || !token) {
                await postSnap.ref.set({ status: "scheduled", updatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
                res.status(200).json({
                    ok: true,
        ...
        if (action === "revokeKey") {
            ...
            await (0, affiliateShared_1.db)().collection("mcpKeyIndex").doc(id).delete();
            res.status(200).json({ ok: true, ...(await loadStudio(affiliateId)) });
            return;
        }
...
//# sourceMappingURL=affiliateTikTok.js.map