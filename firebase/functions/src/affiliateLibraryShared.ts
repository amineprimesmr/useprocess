export const CLIPPING_SITE = "https://useprocess.xyz";

const TIKTOK_POST =
  /(?:https?:\/\/)?(?:www\.|m\.)?tiktok\.com\/@([A-Za-z0-9._]+)\/(?:photo|video)\/(\d+)/i;

export type LocaleText = { fr: string; en: string };

export type ClippingPost = {
  id: string;
  formatId: string;
  url: string;
  cover: string;
...
export type ClippingSpec = {
  ...
  caption: LocaleText;
  fatal: LocaleText;
  slides: Array<{ src: string; fr: string; en: string }>;
  structure: LocaleText[];
};

function text(fr: unknown, en?: unknown): LocaleText {
...
function absAsset(src: string): string {
  const value = String(src || "").trim();
  if (!value) return "";
  if (/^https?:\/\//i.test(value)) return value;
  if (value.startsWith("/")) return `${CLIPPING_SITE}${value}`;
  return value;
}

export function parseTikTokUrl(raw: unknown): { url: string; id: string; handle: string } | null {
...
export function slugFormatId(name: unknown): string {
...
export function seedFormats(seed: {
  FORMAT_LIBRARY?: any[];
  FORMAT_SPECS?: any[];
}): { formats: ClippingFormat[]; specs: ClippingSpec[] } {
  const specs: ClippingSpec[] = (seed.FORMAT_SPECS || []).map((row) => ({
    id: String(row.id || ""),
    ...
    structure: Array.isArray(row.structure)
      ? row.structure.map((item: any) => text(item.fr, item.en))
      : [],
  }));

  const formats: ClippingFormat[] = (seed.FORMAT_LIBRARY || []).map((row) => {
    ...
    return {
      ...
      posts: Array.isArray(row.posts)
        ? row.posts.map((post: any) => seedPost(id, post))
        : [],
    };
  });
...
export function livePostFromDoc(
  id: string,
...
export function liveFormatFromDoc(id: string, data: Record<string, unknown>): ClippingFormat {
  return {
    id,
...
export function mergeCatalog(params: {
  seedFormats: ClippingFormat[];
  seedSpecs: ClippingSpec[];
  liveFormats: ClippingFormat[];
  livePosts: ClippingPost[];
}): { formats: ClippingFormat[]; specs: ClippingSpec[] } {
  const byId = new Map<string, ClippingFormat>();
...
export const MCP_TOOLS = [
  ...
  },
  {
    name: "list_formats",
    description:
      "List EVERY clipping format in the Process library — official Process formats and clipper-created ones. Includes structure specs, post counts, and view totals. Call this before creating or scheduling a TikTok.",
    inputSchema: { type: "object", properties: {} },
  },
  {
    name: "get_format",
    description:
      "Return 100% of one format: name, formula, slide structure, hook, caption, fatal mistakes, example slide images, and every TikTok in that format (URLs, covers, stats).",
    inputSchema: {
      type: "object",
  ...
  },
  {
    name: "list_tiktoks",
    description:
      "List every TikTok in the shared library (official + clipper-added). Filter with format_id. Sorted by views.",
    inputSchema: {
      type: "object",
      properties: {
  ...
  },
  {
    name: "create_format",
    description:
      "Create a new shared format in the Process TikToks library. Other clippers and MCP agents see it immediately.",
    inputSchema: {
      type: "object",
      properties: {
        name_fr: { type: "string" },
  ...
  },
  {
    name: "add_tiktok",
    description:
      "Add a public TikTok URL to a format in the shared library. Hydrates cover/title via oEmbed when possible.",
    inputSchema: {
      type: "object",
      properties: {
        url: { type: "string" },
